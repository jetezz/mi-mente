...
function SettingItem({
  ...
  useEffect(() => {
    const isJson = setting.value !== null && typeof setting.value === 'object';
    const initialMode = isJson ? 'json' : 'text';
    setMode(initialMode);
    setLocalValue(isJson ? JSON.stringify(setting.value, null, 2) : String(setting.value ?? ''));
    setIsDirty(false);
    setError(null);
  }, [setting.value]);
  ...
  const handleSave = () => {
    try {
      let valueToSave: any = localValue;
      if (mode === 'json') {
        valueToSave = JSON.parse(localValue);
      }
      onSave(valueToSave);
      setIsDirty(false);
      setError(null);
      ...
      <div className="flex justify-between items-start mb-3">
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline" size="sm" className="opacity-50">
              {mode.toUpperCase()}
            </Badge>
          </div>
